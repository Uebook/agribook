...
export default function EditYouTubeChannelPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const fetchChannel = async () => {
    try {
      ...
      setFormData({
        ...
        subscriber_count: channel.subscriber_count?.toString() || '',
      ...
      alert(`Error: ${error.message || 'Failed to fetch channel'}`);
  ...
  const handleCategoryToggle = (categoryId: string) => {
    setFormData(prev => ({
      ...
      category_ids: prev.category_ids.includes(categoryId)
        ? prev.category_ids.filter(catId => catId !== categoryId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.updateYouTubeChannel(id, {
        name: formData.name,
      ...
      alert('YouTube channel updated successfully!');
      router.push('/youtube-channels');
    } catch (error: any) {
      console.error('Error updating YouTube channel:', error);
      alert(`Error: ${error.message || 'Failed to update YouTube channel'}`);
    } finally {
      setLoading(false);
    }
  };
        ...
        <main className="flex-1 p-8">
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-3xl font-bold text-gray-900">Edit YouTube Channel</h2>
              <button
                onClick={() => router.back()}
